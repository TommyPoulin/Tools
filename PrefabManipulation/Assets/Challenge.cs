...
public class Challenge : MonoBehaviour
{
    ...
    The objectives are the following
        ...
        1) Create a menu item that generates 20 enemy prefabs and saves them to the project
            DONE a) The enemy prefab contains the following script: BoxCollider, Rigidbody2D, SpriteRenderer, Transform and one randomly selected Old_AIBase child                
            DONE b) Set the enum in the Old_AIBase to a random enum
            DONE c) Link one random scriptable object to it
            DONE d) The enemy AI script should have all elements properly linked
            DONE e) Have a collection of words, and randomly generate a name by mixing 2 of the words
            DONE f) Save the prefab to the project with a randomy generated and refresh the asset database
        ...
        2) Create a menu item that fixes all the bad prefabs. It will loop through and replace all the bad scripts, but maintain the links and values
            DONE a) Loop through all prefabs
            DONE b) Remove the old component and replace it with the good one
            DONE c) Be sure to maintain all links and values
            DONE d) Save the prefab properly, handle all errors and edge cases
