...
{ //Extends Editor required
    ...
    private void Duplicate(int index) {
        ...
        for (int i = wpArrayCount.intValue - 1; i > index + 1; i--)
            SetWaypoint(i, GetWaypointAt(i - 1));
        SetWaypoint(index + 1, GetWaypointAt(index));
    }

    private void ClearNulls() {
        for (int i = wpArrayCount.intValue - 1; i >= 0; i--) {
            if (!GetWaypointAt(i))
                RemoveWaypointAtIndex(i);
        }
    }

    private bool NullsPresent() {
        for (int i = 0; i < wpArrayCount.intValue - 1; i++)
            if (!GetWaypointAt(i))
                return true;
        return false;
    }
    ...
    public override void OnInspectorGUI() //Must be overwritten, this is called everytime the mouse moves over the inspector
    {
        ...
        DropAreaGUI();
        if (GUILayout.Button("Clear Nulls", GUILayout.Width(100f)))
            ClearNulls();

        if (NullsPresent())
            GUILayout.Label("Some lines between waypoints are\nnot rendered due to some\nwaypoints being null.", new GUILayoutOption[] { GUILayout.Width(200f), GUILayout.ExpandHeight(true)});
